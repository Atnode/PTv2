...
<div class="ws_images"><ul>
        <li><a href="/news-2678-le-marioquizz-est-de-retour.html"><img src="images/marioquizz/slider.jpg" alt="MarioQuizz est de retour !" title="MarioQuizz est de retour !" id="wows1_0"/></a></li>
		<li><a href="#"><img src="images/sliders/switch2.png" alt="Bientôt la présentation de la Nintendo Switch !" title="Bientôt la présentation de la Nintendo Switch !" id="wows1_1"/></a></li>
        <li><img src="images/awards2016/awards-resultats-affiche.jpg" alt="DECOUVREZ LES RESULTATS LE SAMEDI 17 DECEMBRE VERS 20H !" title="DECOUVREZ LES RESULTATS LE SAMEDI 17 DECEMBRE VERS 20H !" id="wows1_2"/></li>
	</ul></div>
	<div class="ws_bullets"><div>
		<a href="#" title="MarioQuizz est de retour !"><span>1</span></a>
		<a href="#" title="Votez pour décider du futur avatar de LudaWeb01 !"><span>2</span></a>
	    <a href="#" title="Découvrez les résultats des Planète Toad Awards 2016 le samedi 17 décembre !"><span>3</span></a>
	</div></div>
<div class="ws_shadow"></div>
</div>	
<script type="text/javascript" src="slider/wowslider.js" async defer></script>
</div>
...
$sondage = $query->fetch();
echo'<div id="sondage">
<h3 style="background-color:#8f41af;">Sondage <i class="floatright material-icons">&#xE801;</i></h3><br>';
if ($id!=0) {
...
<h3 style="background-color:#ea4c36;">Communauté <i class="floatright material-icons">people</i></h3>';
...
while ($donnees = $retour->fetch())
{
	...
	if ($TotalDesCommentaires>9) { // News HOT ?>
		<a href="/news-<?php echo $donnees['id']; ?>-<?php echo $titreurl; ?>.html" title="news" style="font-size:1.28rem;margin-left:3px;color:red"><b><?php echo $donnees['titre']; ?></b></a><br><br>
		<?php echo'<span style="font-weight:bold;color:red;"><i class="material-icons md-18">comment</i> '.$TotalDesCommentaires.'</span>';

	} else { //Fin news hot
		if ($recente<604800) {  ?>
		<a href="/news-<?php echo $donnees['id']; ?>-<?php echo $titreurl; ?>.html" title="news" style="font-size:1.28rem;margin-left:3px;color:darkgreen"><b><?php echo $donnees['titre']; ?></b></a><br><br>
		<?php } else { ?>
		<a href="/news-<?php echo $donnees['id']; ?>-<?php echo $titreurl; ?>.html" title="news" style="font-size:1.28rem;margin-left:3px;color:#424242;"><b><?php echo $donnees['titre']; ?></b></a><br>
		<?php }
		echo'<hr><br><div><div class="news-'.$image = $choix_icon[$donnees['icon']].'">'.$image = $choix_icon[$donnees['icon']].'</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:grey;"><i class="material-icons md-18">comment</i> '.$TotalDesCommentaires.'</span></div>';
	}
	echo '</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br>';
	$NNews++; if($NNews==3) { echo'</div><br><div class="tableNews">'; $NNews = 0; }
}
?>
</div><br><a href="/archives-news.html" title="Archives des news"><div class="archiveNews">Voir plus de news</div></a><br>
<?php if ($id!=0) {
...
<?php $jeuxRsortis = $db->prepare('SELECT * FROM jeux ORDER BY sortie_ue DESC LIMIT 12');
$jeuxRsortis->execute(); ?>
<div class="top-jeux">
<h3 style="background-color:#d65400;">Jeux récemment sortis <i class="floatright material-icons">date_range</i></h3>
...
<div class="chronique">
<h3 style="background-color:#1b7fbb;">Chroniques <i class="floatright material-icons">create</i></h3>
<?php
...
<div class="commentaires" style="padding:20px;"><table>
    <tbody>
      <tr>
        <td style="width:40px;">
          <img src="'.$avatar.'" alt="avatar" width="40" height="40" style="border-radius:50%;margin-top:5px;" />
        </td>';
     include("code.php");
     echo'<td><textarea name="publier" rows="2" cols="60" id="message" placeholder="Publier un p\'tit message..."></textarea></td><td>
