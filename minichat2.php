...
//
if ($lvl<5 AND $id!="55582") header('Location: ../erreur_403.html'); // C EST UNE BETA DU CHAT DONC SEULS
// LES ADMINS PEUVENT
...
{
echo'<div class="blockChat">
<div style="float:left;width:75%;height: calc(100% - 110px);overflow-y:scroll;" id="minichat">';
if ($lvl==0) {
...
(
  SELECT *
  FROM forum_chat
  LEFT JOIN forum_membres ON membre_id = posteur_id
  ORDER BY id DESC
  LIMIT 30
) x
  ORDER BY id ASC');
        ...
        <tbody>
          <tr>
            ...
            if (!empty($val['membre_avatar'])) {
              echo '<a href="/profil-'.$val['membre_id'].'.html"><img src="'.$val['membre_avatar'].'" style="border-radius:50%;margin-top:10px;" width="50" title="'.$val['membre_pseudo'].'" /></a>' ;
            }
            echo '</td>
...
echo'<div style="height: calc(100% - 110px);overflow-y:scroll;" id="online"><h2>Membres en ligne</h2>
<table><tbody><tr>';
...
echo'
</tr></tbody></table></div><div class="clearboth"></div><br>
<p style="text-align:center;">
<form action="#" method="post" style="margin-left:auto;margin-right:auto;text-align:center;">';
echo'<br>
<table style="left:25%;position:absolute;"><tbody><tr>
 <td>';
include("code.php");
 echo'<textarea name="message" rows="2" cols="60" id="message" placeholder="Votre message..."></textarea>
 </td><td>
 <br><button id="envoyer" class="mdl-button mdl-js-button mdl-button--raised mdl-button--colored" type="button"><i class="material-icons">&#xE163;</i> Envoyer (ou CTRL+Entrée)</button></td>
</tr></tbody></table>
</form></p>
</div>
';
...
function e(fromMe) {
    ...
    element.scrollTop = element.scrollHeight;
        ...
        function fonline() {
            $("#online").html(), $("#online").load("minichat_online2.php")
        }
    ...
    $(function() {
        // Sections height
            ...
            $('.blockChat').css('height', sH + 'px').css('height', '-=83px');
