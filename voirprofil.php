...
session_start();
	...
	if ($reponse->rowCount()<1) { header("HTTP/1.0 404 Not Found"); }
  $donnees = $reponse->fetch();
	...
	if ($query->rowCount()<1)
	{
		...
		if ($data['description']!=NULL) {echo '<br><div style="text-align:center;"><i>“'.code(htmlspecialchars($data['description'])).'”</i></div><br><br>';}
		...
		//Infos			
         ...
         echo'<div class="infop"><br><img src="'.$rang['name'].'" alt="Rang du membre" width="200" height="33" /><br><br><div style="float:left;">';
			...
			<p><strong>Code-Ami 3DS :</strong> '.htmlspecialchars($data['ca_3ds']).'</p>
			<p><strong>Code-Ami Nintendo Switch :</strong> SW-'.htmlspecialchars($data['ca_switch']).'</p>
			<p><strong>Identifiant Nintendo Network :</strong> <a href="https://miiverse.nintendo.net/users/'.htmlspecialchars($data['nintendo_network']).'">'.htmlspecialchars($data['nintendo_network']).'</a></p>
...
<div class="commentaires" style="padding:20px;"><table>
 ...
 echo'<div id="publimembre">';
       $query=$db->prepare('SELECT * FROM publications LEFT JOIN forum_membres ON forum_membres.membre_id = publications.id_receveur WHERE id_receveur = :membre ORDER BY timestamp DESC'); // On sélectionne les publications
       $query->bindValue(':membre',$membre, PDO::PARAM_INT);
       $query->execute();
 if ($query->rowCount()>0)
 {
...
if ($dataP['id_posteur'] == $dataP['id_receveur']) { // Si le membre poste sur son profil
    echo '<div class="commentaires" style="padding:20px;"><table>
      ...
      if ($dataP['officielle']==0) {
        ...
        <a href="/profil-'.$dataP['membre_id'].'.html" style="color:'.$dataP['membre_couleur'].';">'.$dataP['membre_pseudo'].'</a> a publié sur son mur</p><a href="/publi-'.$publi.'.html" style="color:#a0a0a0;"><i class="material-icons md-18" style="font-size:14px;">av_timer</i>
        ...
        echo'<td style="width:98%; text-align:left;padding-left:15px;"><span style="font-size:14px;"><p style="color:#888;">'.code(stripslashes(htmlspecialchars($dataP['message']))).'</p><p style="color:#a0a0a0;"><i class="material-icons md-18" style="font-size:14px;">av_timer</i>
