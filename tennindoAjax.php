...
$commentaire=$db->prepare('SELECT * FROM commentairest LEFT JOIN forum_membres ON membre_id = id_posteur WHERE id_news = '. $news .' ORDER BY timestamp DESC'); // On sélectionne les publications
$commentaire->execute();
...
if ($data['id_posteur']==$id OR $lvl>3){echo'<div style="text-align:right;font-style:italic;"><a href="./commt-suppr.php?id='.$data['id'].'"><img src="./images/croix-suppr.png" alt="Supprimer un message" title="Supprimer un message" width="16" height="16"/></a></div>';}
echo'</tr></table>
